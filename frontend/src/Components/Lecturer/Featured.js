...
course_no += `${mail[i]}`
  ...
  useEffect(() => {
    if(isloading == true) {
      try {
      axios.post('http://localhost:8082/student/details', { "course_name" : `Course${course_no}` })
      ...
      .then(data => {
        ...
        try {
          axios.post('http://localhost:8082/student/count', { "course_name" : `Course${course_no}`, date : data[data.length - 1].date })
        ...
        try {
          axios.post('http://localhost:8082/student/count', { "course_name" : `Course${course_no}`, status : "p", date : data[data.length - 1].date})
