...
export default function Featured() {
  ...
  useEffect(() => {
    if(isloading == true) {
      try {
      axios.post('http://localhost:8082/student/details', { "roll_no" : `${roll_no}`, "branch" : `${branch}` })
      ...
      .then(data => {
        ...
        try {
          axios.post('http://localhost:8082/student/count', { "roll_no" : `${roll_no}`, "branch" : `${branch}`, date : data[data.length - 1].date })
        ...
        try {
          axios.post('http://localhost:8082/student/count', { roll_no : `${roll_no}`, status : "p", date : data[data.length - 1].date})
